---
title: 2020-07-25-transformer代码
date: 2020-07-25 10:40:24
tags: transformer
categories: transformer
description: transformer代码注解
---
...
tfds.load :加载数据集.1.下载数据并将其另存为tfrecord文件.   2.加载tfrecord并创建tf.data.Dataset.
metadata: 元数据 用于描述数据的数据，比如数码照片的EXIF信息，它就是一种用来描述数码图片的元数据
...
# 过滤过长的数据
train_dataset = train_dataset.filter(filter_long_sent) # 这个函数为什么不加（） ？？？
# 使用缓存数据加速读入
...
![$$\Large{PE_{(pos, 2i)} = sin(pos / 10000^{2i / d_{model}})} $$$$\Large{PE_{(pos, 2i+1)} = cos(pos / 10000^{2i / d_{model}})} $$](https://render.githubusercontent.com/render/math?math=%5CLarge%7BPE_%7B%28pos%2C%202i%29%7D%20%3D%20sin%28pos%20%2F%2010000%5E%7B2i%20%2F%20d_%7Bmodel%7D%7D%29%7D%20%24%24%24%24%5CLarge%7BPE_%7B%28pos%2C%202i%2B1%29%7D%20%3D%20cos%28pos%20%2F%2010000%5E%7B2i%20%2F%20d_%7Bmodel%7D%7D%29%7D&mode=display)
...
def positional_encoding(position, d_model):
	#numpy.arange(start, stop, step, dtype = None) np.arange(position) = [0,...,position-1]
    angle_rads = get_angles(np.arange(position)[:, np.newaxis],   # shape(position,1)
                           np.arange(d_model)[np.newaxis,:],  # shape(1,d_model)
                           d_model)
    # 第2i项使用sin
    sines = np.sin(angle_rads[:, 0::2])  #从0开始，步长为2
    # 第2i+1项使用cos
    cones = np.cos(angle_rads[:, 1::2]) #从1开始，步长为2
...
def create_padding_mark(seq):
    # 获取为0的padding项
    seq = tf.cast(tf.math.equal(seq, 0), tf.float32) #将seq的编码逐个与0比较，相等为1不等为0
...
![img](https://camo.githubusercontent.com/22371733ed7ffa9065a60e80d3b83b10d832072a/68747470733a2f2f7777772e74656e736f72666c6f772e6f72672f696d616765732f7475746f7269616c732f7472616e73666f726d65722f7363616c65645f617474656e74696f6e2e706e67)进行attention计算的时候有3个输入 Q (query), K (key), V (value)。计算公式如下：![$$\Large{Attention(Q, K, V) = softmax_k(\frac{QK^T}{\sqrt{d_k}}) V} $$](https://render.githubusercontent.com/render/math?math=%5CLarge%7BAttention%28Q%2C%20K%2C%20V%29%20%3D%20softmax_k%28%5Cfrac%7BQK%5ET%7D%7B%5Csqrt%7Bd_k%7D%7D%29%20V%7D&mode=display)
...
在这里我们将`被掩码的token乘以-1e9(表示负无穷)`,这样softmax之后就为0,不对其他token产生影响。
...
def scaled_dot_product_attention(q, k, v, mask):
    # query key 相乘获取匹配关系
    matmul_qk = tf.matmul(q, k, transpose_b=True)  # transpose_b=True表示b项在运算前要进行转置运算
...
![img](https://camo.githubusercontent.com/5464abc4535579fdb88afb7117617f6f23ff50af/68747470733a2f2f7777772e74656e736f72666c6f772e6f72672f696d616765732f7475746f7269616c732f7472616e73666f726d65722f6d756c74695f686561645f617474656e74696f6e2e706e67)
...
每个多头注意块有三个输入; Q（查询），K（密钥），V（值）。 它们`通过第一层线性层并分成多个头`。

注意:点积注意力时需要使用mask， 多头输出需要使用tf.transpose调整各维度。

Q，K和V不是一个单独的注意头，而是分成多个头，因为`它允许模型共同参与来自不同表征空间的不同信息。 在拆分之后，每个头部具有降低的维度，总计算成本与具有全维度的单个头部注意力相同。`

python 中`assert断言`是声明其布尔值必须为真的判定，如果发生异常就说明表达式为假。可以理解assert断言语句为**raise-if-not**，用来测试表示式，其返回值为假，则会抛出AssertError并且包含错误信息。如果它为真，就不做任何事
...
![img](https://camo.githubusercontent.com/ce5d9e0b508ffa8c84c943258c677b7576f35757/68747470733a2f2f7777772e74656e736f72666c6f772e6f72672f696d616765732f7475746f7269616c732f7472616e73666f726d65722f7472616e73666f726d65722e706e67)
...
带自定义学习率调整的Adam优化器![$$\Large{lrate = d_{model}^{-0.5} * min(step{\_}num^{-0.5}, step{\_}num * warmup{\_}steps^{-1.5})}$$](https://render.githubusercontent.com/render/math?math=%5CLarge%7Blrate%20%3D%20d_%7Bmodel%7D%5E%7B-0.5%7D%20%2A%20min%28step%7B%5C_%7Dnum%5E%7B-0.5%7D%2C%20step%7B%5C_%7Dnum%20%2A%20warmup%7B%5C_%7Dsteps%5E%7B-1.5%7D%29%7D&mode=display)
